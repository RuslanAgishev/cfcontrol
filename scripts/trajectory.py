...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        for z in range(10):
            msg.x = crazyflie18.transform.translation.x
            msg.y = crazyflie18.transform.translation.y
            msg.yaw = 0.0
            msg.z = z / 25.0
            now = rospy.get_time()
            msg.header.seq += 1
            ...
            #msg.x = crazyflie15.transform.translation.x
            #msg.y = crazyflie15.transform.translation.y
            #msg.header.stamp = rospy.Time.now()
    ...
    x = tr[:,0]  ; y = tr[:,1]
    X = np.insert(x, 2, np.linspace(x[1], x[2], 50))
    Y = np.insert(y, 2, np.linspace(y[1], y[2], 50))
    X = np.insert(X, -3, np.linspace(x[-4], x[-3], 50))
    Y = np.insert(Y, -3, np.linspace(y[-4], y[-3], 50))
