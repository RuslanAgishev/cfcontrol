...
def tag_game(human, cf1, cf2, cf3, obstacle1, obstacle2):
	...
	drone1_pose_goal = np.array([  human_pose[0] - 2.0*l       ,
								   human_pose[1]               ,
								   human_pose[2] + 0.1         ])
	...
	flew_in1, closest_obstacle1 = swarmlib.obstacle_status(Obstacles_map, drone1_pose_goal_prev, imp_pose_from_theta, human_pose, R_obstacles, flew_in1)
	# in the circle-like vicinity of the obstacle
	if flew_in1 > 0:
		if flew_in1 == 1: # crossed the circle near the obstacle
			print "Drone 1 is near the obstacle "+str(closest_obstacle1+1)
			imp_theta = theta1_sp
		...
		imp_pose_from_theta = Obstacles_map[closest_obstacle1] + np.array([R_obstacles*np.cos(imp_theta), R_obstacles*np.sin(imp_theta)])
		if theta_imp:
	...
	flew_in2, closest_obstacle2 = swarmlib.obstacle_status(Obstacles_map, drone2_pose_goal_prev, imp_pose_from_theta2, human_pose, R_obstacles, flew_in2)
	# in the circle-like vicinity of the obstacle
	if flew_in2 > 0:
		if flew_in2 == 1: # crossed the circle near the obstacle
			print "Drone 2 is near the obstacle "+str(closest_obstacle2+1)
		...
		imp_pose_from_theta2 = Obstacles_map[closest_obstacle2] + np.array([R_obstacles*np.cos(imp_theta2), R_obstacles*np.sin(imp_theta2)])
		if theta_imp:
	...
	flew_in3, closest_obstacle3 = swarmlib.obstacle_status(Obstacles_map, drone3_pose_goal_prev, imp_pose_from_theta3, human_pose, R_obstacles, flew_in3)
	# in the circle-like vicinity of the obstacle
	if flew_in3 > 0:
		if flew_in3 == 1: # crossed the circle near the obstacle
			print "Drone 3 is near the obstacle "+str(closest_obstacle3+1)
		...
		imp_pose_from_theta3 = Obstacles_map[closest_obstacle3] + np.array([R_obstacles*np.cos(imp_theta3), R_obstacles*np.sin(imp_theta3)])
		if theta_imp:
